...
if (!nexacro.ChartPieSeriesControl) {
	...
	_pChartPieSeriesControl.highlightfilltype = true;
	_pChartPieSeriesControl.autogradation = "none";
	...
	_pChartPieSeriesControl.on_apply_highlightradius = function (highlightradius) {
		if (!nexacro._ChartGraphicsLib.isEmpty(highlightradius)) {
	...
	_pChartPieSeriesControl.set_highlightfilltype = function (val) {
		if (val === undefined || val === null) {
			val = true;
		}
		val = nexacro._toBoolean(val);
		if (this.highlightfilltype != val) {
			this._changeContentsProperty("highlightfilltype", val, this.highlightfilltype);
			this.highlightfilltype = val;
			this.on_apply_highlightfilltype(val);
		}
		this._chart._draw();
	};

	_pChartPieSeriesControl.on_apply_highlightfilltype = function (highlightfilltype) {
	};

	_pChartPieSeriesControl.set_autogradation = function (val) {
		if (val === undefined || val === null) {
			val = "none";
		}
	...
	_pChartPieSeriesControl.on_apply_autogradation = function (autogradation) {
		this._redrawSeries = false;
	};
	...
	_pChartPieSeriesControl._processItemData = function (data) {
		var length = data.length, slice = [], i, value, total = 0;
	...
	_pChartPieSeriesControl._drawPie = function () {
		...
		if (nexacro._ChartGraphicsLib.isEmpty(this.innerradius)) {
		}
		else {
		...
		function drawSlice (pThis, i) {
			if (items[i]._isShow) {
				...
				var startAngle = Math.round(rStartAngle * 180 / Math.PI), endAngle = Math.round(rEndAngle * 180 / Math.PI), highlightStartAngle = rHighlightStartAngle * 180 / Math.PI, highlightEndAngle = rHighlightEndAngle * 180 / Math.PI, w = radius, h = radius, p0, p1, p2, p3, la, pathData, index = 0, dx = 0, dy = 0, centerX = pThis._chart._centerLeft, centerY = pThis._chart._centerTop, selectindent = 0, startangle = pThis._startangle || 0;

				if (pThis.selectindent < 0 || nexacro._ChartGraphicsLib.isEmpty(pThis.selectindent)) {
				}
				else {
				...
				if (pThis.highlightvisible) {
					...
					if (pThis.highlightfilltype) {
						...
						innerw = innerh = hRadius + 3;

						p2 = (Math.sin(rHighlightStartAngle) * innerw + centerX) + " " + (-Math.cos(rHighlightStartAngle) * innerh + centerY);
						p3 = (Math.sin(rHighlightEndAngle) * innerw + centerX) + " " + (-Math.cos(rHighlightEndAngle) * innerh + centerY);

						if (items.length == 1) {
							highlightPathData += " L" + p3 + " A" + innerw + " " + innerw + " 1" + la + " 1 " + p2 + " Z";
						}
						else {
							highlightPathData += " L" + p3 + " A" + innerw + " " + innerw + " 1" + la + " 0 " + p2 + " Z";
						}
					}
				...
				var bTarget = false;
				if (nexacro._Browser != "Runtime" && (pThis.autogradation != "none")) {
					...
					if (bTarget) {
						...
						var gColorObj = slice.fillstyle = slice._style.fillstyle;
						var gradient = gColorObj.gradient;
		...
		function drawSliceBorder (pThis, i) {
			if (items[i]._isShow) {
				...
				var startAngle = Math.round(rStartAngle * 180 / Math.PI), endAngle = Math.round(rEndAngle * 180 / Math.PI), w = radius, h = radius, p0, p1, p2, p3, la, pathData, index = 0, centerX = pThis._chart._centerLeft, centerY = pThis._chart._centerTop;
	...
	_pChartPieSeriesControl._drawLabels = function () {
		...
		function drawLabel (item, startangle, index, pThis) {
			var halfAngle = 0, maxRadius = Math.min(pThis._chart._boardWidth, pThis._chart._boardHeight) / 2, centerX = pThis._chart._centerLeft, centerY = pThis._chart._centerTop, itemText, itemtextguidesize = pThis._itemtextguidesize, textX = 0, textY = 0, itempointsize, pointx, pointy, itemtextguidelinestyle = pThis.itemtextguidelinestyle, itemtextguidelineopacity = pThis.itemtextguidelineopacity, guideline, seriesGroup = pThis._chart._seriesGroup, itemtextguidelocation = pThis._itemtextguidelocation, itemId, radius, targetX = 0, targetY = 0, borderwidth = pThis._borderwidth;

			if (nexacro._ChartGraphicsLib.isEmpty(pThis.radius)) {
				radius = maxRadius * 0.8;
			}
			else {
			...
			if (!nexacro._isNull(itemText)) {
				...
				if (itemtextguidelocation == "out") {
					var centerAngle = halfAngle * 180 / Math.PI, ptPos = {
						x : pointx, 
						y : pointy
					}, originpoint = {
						x : centerX, 
						y : centerY
					}, vUnit, rotateX = 0, rotateY = 0, targetX, targetY;
	...
	_pChartPieSeriesControl._hideHighlight = function (item) {
		...
		if (highlight && !nexacro._ChartGraphicsLib.isEmpty(highlight.parent)) {
			this._chart._chageGroupObject(this._chart._seriesGroup, this._chart._highlightGroup, true);
	...
	_pChartPieSeriesControl._drawHighlight = function (item) {
		...
		if (item) {
			...
			if (highlightPathData) {
				...
				if (this.highlightfilltype == true) {
					slice.set_fillstyle(highlightfillstyle);
				}
				else {
					if (item._highlightcolor) {
						slice.set_fillstyle(item._highlightcolor);
					}
					else {
						slice.set_fillstyle(item._style.fillstyle);
					}
				}
				slice.set_strokejoin("round");
