...
if (nexacro.Edit) {
	nexacro._defineImeLocaleEdit("ja", {
		"Edit" : {
			...
			on_keydown_basic_specialkey_process : function (keycode, alt_key, ctrl_key, shift_key) {
				var input_elem = this._input_element;
				if (keycode == nexacro.KeyCode_ImeInput && this._imedisable || (ctrl_key && keycode == 86 && input_elem.isComposing())) {
					this._killfocus_fix_composition();
			...
			on_keydown_basic_process : function (keycode, alt_key, ctrl_key, shift_key) {
				var input_elem = this._input_element;
		...
		"TextArea" : {
			...
			on_keydown_basic_specialkey_process : function (keycode, alt_key, ctrl_key, shift_key) {
				var input_elem = this._input_element;
				if ((keycode == nexacro.KeyCode_ImeInput && this._imedisable) || (ctrl_key && input_elem.isComposing() && keycode == 86)) {
					this._killfocus_fix_composition();
					var input_handle = input_elem.handle;
					if (nexacro._Browser == "Chrome") {
						input_handle.blur();
			...
			on_keydown_default_specialkey_process : function (keycode, alt_key, ctrl_key, shift_key, refer_comp) {
				var input_elem = this._input_element;
				...
				return nexacro.TextArea.prototype.on_keydown_default_specialkey_process.call(this, keycode, alt_key, ctrl_key, shift_key, refer_comp);
			}, 
			on_keyinput_basic_before_process : function (input_text) {
				var input_elem = this._input_element;
				var input_value = input_elem.value === null ? undefined : input_elem.value;
				...
				if (!this._onlydisplay) {
					...
					if (input_elem.isComposing()) {
						...
						if (caretpos && caretpos != -1) {
							pos = [caretpos.begin, caretpos.end];
						}
					}
		...
		nexacro._defineImeLocaleEdit("ja", {
			"InputElement" : {
				...
				setCompositionComplete_process : function (end_pos, is_composing) {
					...
					_is_composing = is_composing || this._composer.isComposing();
					if (_is_composing) {
				...
				_on_sys_keyinput_process_use_event_beforeinput : function (evt) {
					...
					if (pThis._beforeinput_result && (!pThis._composer.isComposing() || evt.inputType == "insertFromPaste")) {
						var bcancle = false;
						pThis._beforeinput_result.forEach(function (state) {
							switch (state) {
				...
				on_sys_compositionupdate_process : function (evt) {
					...
					this._composer.setStatus(nexacro._CompositionState.COMPOSING, pos.end, false);
					return true;
				}, 
				...
				on_sys_compositionend_process : function (evt) {
					...
					if (pThis._is_focused) {
						...
						if (pThis.autoskip && pThis._is_focused) {
							if (ismax) {
								if (nexacro._Browser == "Chrome" || (nexacro._Browser == "IE" && (nexacro._BrowserType == "Edge" || nexacro._BrowserVersion == 11))) {
									pThis._go_next_focus();
								}
								else {
									setTimeout(function () {
				...
				on_complete_composition_value_process : function () {
					if (nexacro._Browser === "Gecko" || nexacro._Browser === "IE") {
						if (this.handle) {
							this.handle.blur();
							this.handle.focus();
