...
if (!nexacro.Combo) {
	...
	_pCombo._apply_setfocus = function (evt_name) {
		...
		if (comboedit) {
			...
			if (control_elem && nexacro._isTouchInteraction && nexacro._SupportTouch && this.type == "dropdown") {
	...
	_pCombo._on_value_change = function (preindex, pretext, prevalue, postindex, posttext, postvalue) {
		...
		if (this.applyto_bindSource("value", postvalue)) {
			...
			if (postvalue == prevalue && postindex != preindex) {
				this.value = postvalue;
				this.text = posttext;
				this.index = postindex;
			}
		...
		if (ds && comboedit._processing_keyfilter) {
			...
			switch (type) {
				...
				case "caseifilterlike":
					if (this.type == "filter" || this.type == "caseifilter" || this.type == "caseifilterlike") {
						var regExp;
						var parse_val = "";
						...
						if (this.type == "filter") {
							...
							ds.set_filterstr(col + ".match(/^(" + parse_val + ")/i)");
						}
						else {
							ds.set_filterstr(col + ".match(/(" + parse_val + ")/i)");
						}
	...
	_pCombo._showPopup = function (ds, index) {
		...
		if (ret) {
			...
			if (rowcnt != this._innerdataset.rowcount) {
				index = this.index;
			}
