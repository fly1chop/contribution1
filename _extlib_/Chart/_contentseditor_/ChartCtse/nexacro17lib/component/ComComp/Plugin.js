...
if (!nexacro.Plugin) {
	...
	_pPlugin.callMethod = function () {
		var obj_elem = this._obj_elem;
		var array = Object.assign(arguments);

		if (obj_elem) {
			for (var i = 0; i < array.length; i++) {
				if (array[i] instanceof nexacro.Plugin) {
					var plugin_elem = array[i]._obj_elem;
					if (plugin_elem) {
						array[i] = plugin_elem.getPluginObject();
					}
				}
			}
			return obj_elem.callMethod.apply(obj_elem, array);
		}
	};
