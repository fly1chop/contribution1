...
if (!nexacro.Calendar) {
				...
				if (keycode == nexacro.Event.KEY_ESC) {
					...
					if (!this._isPopupVisible()) {
						...
						if (calendaredit) {
							...
							this.datepicker.on_apply_value(value);
						}
					}
				}
				else {
	...
	_pCalendar._on_drop_onlbuttondown = function () {
		...
		if (this._isPopupVisible()) {
			...
			if (datepicker && calendaredit) {
				...
				datepicker.on_apply_value(value);
			}
		...
		if (calendaredit) {
			...
			if (maskobj) {
				if (!maskobj.date) {
					...
					this._setSpinValue(cur_val, cur_text, old_pos);
				}
				else {
	...
	_pCalendar._isValidDate = function (val, editmask_type) {
		var y, M, d;
		var h, m, s, ss;
