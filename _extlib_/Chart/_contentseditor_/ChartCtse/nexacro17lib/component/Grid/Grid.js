...
if (!nexacro.Grid) {
		...
		if (this.__update(status, onlycontents, nochk_updatearea)) {
			...
			if (control_elem) {
				...
				if (grid._isFakeCell(datarow) == false && !suppproc) {
					this._virtualmerge = grid._checkVirtualMerge(cellinfo, datarow);

					if (this._virtualmerge) {
						var remove = this._virtualmerge.remove;
	...
	_pGridCell._showEditor = function (focus, showfull) {
		...
		var datarow = grid._getDataRow(this._rowidx);
		var editComp;
		var evt_name = nexacro._isTouchInteraction ? "touch" : "";
	...
	_pGridCalendar._on_edit_oneditclick = function (obj, e) {
		...
		if (this._type == "system") {
			...
			if (control_elem) {
				...
				nexacro._openSystemCalendar(calendar ? calendar : this, this.value, "_on_notify_mobile_valuechanged");
			}
		}
		return ret;
	};
			...
			if (_reason == 1 || (_reason == 0 && e.progressload)) {
				if (this.autosizingtype != "none") {
					if (this._async_create == true) {
						this._recreate_contents_all_async(true, false, true, undefined, prevrowcnt);
					}
					else {
						this._recreate_contents_all(true, false, true, undefined, prevrowcnt);
					}
				}
				else {
	...
	_pGrid._getCellStyleInfo = function (cellidx, prop, datarow, selected, parent_cellinfo) {
					...
					if (autosizecol == "none") {
						...
						if (displayType == "checkboxcontrol") {
							...
							if (controlSize == undefined) {
								controlSize = this._getCellStyleInfo(cells[i]._cellidx, "checkboxsize", row, false, parent_cellinfo);
	...
	_pGrid._getCellRowTextSize = function (cellinfo, rowidx, text, parent_cellinfo, only_normal, merge_width) {
		...
		if (!word) {
			word = this._getCellStyleInfo(cellinfo._cellidx, "wordWrap", rowidx, undefined, parent_cellinfo);
		}

		if (font === undefined) {
			font = this._getCellStyleInfo(cellinfo._cellidx, "font", rowidx, undefined, parent_cellinfo);
		...
		if (select_font === undefined) {
			select_font = this._getCellStyleInfo(cellinfo._cellidx, "font", rowidx, true, parent_cellinfo);
		...
		if (displayType == "imagecontrol") {
			...
			var style = this._getCellStyleInfo(cellinfo._cellidx, ["wordSpacing", "letterSpacing"], rowidx, undefined, parent_cellinfo);
			...
			if (usewordwrap && (this._autoSizeRowProc || this._rowSizeEx) && word != null && word != "none" && word != false && word != "false") {
				...
				if (merge_width != undefined) {
					width = merge_width;
					padd = this._getCellStyleInfo(cellinfo._cellidx, "padding", rowidx, undefined, parent_cellinfo);
					if (padd) {
						padd = new nexacro._PaddingObject(padd);
					...
					style = this._getCellStyleInfo(cellinfo._cellidx, ["padding", "border"], rowidx, undefined, parent_cellinfo);

					padd = style.padding;
					var bord = style.border;
					var select_bord = this._getCellStyleInfo(cellinfo._cellidx, "border", rowidx, true, parent_cellinfo);
				...
				if (subcells.length > 0) {
					...
					if (autosizerow == "none") {
						...
						if (displayType == "checkboxcontrol") {
							...
							if (controlSize == undefined) {
								controlSize = this._getCellStyleInfo(cells[i]._cellidx, "checkboxsize", rowidx, false, parent_cellinfo);
							...
							if (padd === "bindexpr" || padd === undefined) {
								padd = this._getCellStyleInfo(i, "padding", rowidx, undefined, parent_cellinfo);
							}

							if (bord === "bindexpr" || bord === undefined) {
								bord = this._getCellStyleInfo(i, "border", rowidx, undefined, parent_cellinfo);
							}
	...
	_pGrid._getDisplaytypeControlSize = function (b_col, displaytype, cellinfo, parent_cellinfo, rowidx) {
		...
		var controlSize = this._getCellStyleInfo(cellinfo._cellidx, "controldisplaysize", rowidx, false, parent_cellinfo);
	...
	_pGrid._getDepthWidth = function (datarow, cellinfo) {
		...
		if (image) {
			...
			image = nexacro._getImageLocation(image, this._getRefFormBaseUrl());
			size = nexacro._getImageSize(image, this._on_treeloadImagetemp, this);
		...
		if (this._tempmergeeditor) {
			while (true) {
				if (this.__moveToCell(type, editable, colmove, area, lastcol, showcell)) {
					...
					afterSubrow = this._selectinfo.cur;
	...
	_pGrid._hideEditorMergeCell = function (after_hideeditor) {
		...
		if (!after) {
			...
			if (this._tempmergeeditor == after_hideeditor) {
