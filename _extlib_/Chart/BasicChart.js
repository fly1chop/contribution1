...
if (!nexacro.BasicChart) {
	...
	_pBasicChart.stacktotalvisible = false;
	_pBasicChart.stacktotallinestyle = null;
	_pBasicChart.stacktotalshadowstyle = null;
	_pBasicChart.stacktotaltextcolor = null;
	_pBasicChart._stacktotallinestyle = null;
	_pBasicChart._stacktotalshadowstyle = null;
	_pBasicChart._stacktotaltextcolor = null;
	...
	_pBasicChart.on_destroy_contents = function () {
		...
		this.stacktotalvisible = null;
		this.stacktotallinestyle = null;
		this.stacktotalshadowstyle = null;
		this.stacktotaltextcolor = null;
		this._stacktotallinestyle = null;
		this._stacktotalshadowstyle = null;
		this._stacktotaltextcolor = null;
	...
	_pBasicChart.set_stacktotalvisible = function (val) {
		val = nexacro._toBoolean(val);

		if (this.stacktotalvisible != val) {
			this.stacktotalvisible = val;
			this.on_apply_stacktotalvisible(val);
		}

		if (this.stacktype != "normal") {
			this.set_stacktype("normal");
			return;
		}
		this._reset = true;
		this._draw();
	...
	_pBasicChart.on_apply_stacktotalvisible = function (val) {
		this._changedData = true;
	};

	_pBasicChart.set_stacktotallinestyle = function (val) {
		if (val == "") {
			val = null;
		}
		this.stacktotallinestyle = val;
		if (val) {
			if (this._stacktotallinestyle == null || this._stacktotallinestyle.value != val) {
				var stacktotallinestyle = nexacro.BorderLineObject(val);
				this._stacktotallinestyle = stacktotallinestyle;
				this.on_apply_stacktotallinestyle(stacktotallinestyle);
			}
		}
		else {
			if (this._stacktotallinestyle) {
				this._stacktotallinestyle = null;
				this.on_apply_stacktotallinestyle(null);
			}
		}
		this._reset = true;
		this._draw();
	...
	_pBasicChart.on_apply_stacktotallinestyle = function (val) {
	};
	_pBasicChart.set_stacktotalshadowstyle = function (val) {
		if (val == "") {
			val = null;
		}
		this.stacktotalshadowstyle = val;
		if (val) {
			if (this._stacktotalshadowstyle == null || this._stacktotalshadowstyle.value != val) {
				var stacktotalshadowstyle = nexacro.ShadowObjectNew(val);
				this._stacktotalshadowstyle = stacktotalshadowstyle;
				this.on_apply_stacktotalshadowstyle(stacktotalshadowstyle);
			}
		}
		else {
			if (this._stacktotalshadowstyle) {
				this._stacktotalshadowstyle = null;
				this.on_apply_stacktotalshadowstyle(null);
			}
		}
		this._reset = true;
		this._draw();
	...
	_pBasicChart.on_apply_stacktotalshadowstyle = function (val) {
	};
	_pBasicChart.set_stacktotaltextcolor = function (val) {
		this.stacktotaltextcolor = val;
		if (val) {
			if (this._stacktotaltextcolor == null || this._stacktotaltextcolor.value != val) {
				var stacktotaltextcolor = nexacro.ColorObject(val);
				this._stacktotaltextcolor = stacktotaltextcolor;
				this.on_apply_stacktotaltextcolor(stacktotaltextcolor);
			}
		}
		else {
			if (this._stacktotaltextcolor) {
				this._stacktotaltextcolor = null;
				this.on_apply_stacktotaltextcolor(null);
			}
		}
		this._reset = true;
		this._draw();
	...
	_pBasicChart.on_apply_stacktotaltextcolor = function (val) {
		this._changedData = true;
	};
	...
	_pBasicChart._getVisibleStackGroupingLength = function () {
		...
		if (stackbargroup.length > 1) {
			stackbargroup = this._checkStackEliminateDuplicate(stackbargroup);
		}
	...
	_pBasicChart._setDatapoint = function () {
		...
		if (this._barVisibleSeriesCnt > 1 && this._bargrouping) {
			var chartBarWidth = this._chartbarsize;
	...
	_pBasicChart._setStackPercent = function (series) {
		...
		this._stackPercents = [];
		nexacro._ChartGraphicsLibArray.forEach(data, function (d, i) {
	...
	_pBasicChart._getSumsStackPercent = function (stackgroupseries) {
		...
		nexacro._ChartGraphicsLibArray.forEach(series, function (s, i) {
			...
			nexacro._ChartGraphicsLibArray.forEach(s._data, function (d, j) {
				var value = 0, key;
	...
	_pBasicChart._setGroupColumnDatapoints = function (series) {
		...
		function getAxeMinMaxValues (datapoints) {
			...
			if (rotateaxis) {
				if (nexacro._ChartGraphicsLib.isNumber(ymin)) {
					minMaxValues[0] = Number(ymin);
				}

				if (nexacro._ChartGraphicsLib.isNumber(ymax)) {
					minMaxValues[1] = Number(ymax);
				}
			}
			else {
				if (nexacro._ChartGraphicsLib.isNumber(xmin)) {
					minMaxValues[0] = Number(xmin);
				}

				if (nexacro._ChartGraphicsLib.isNumber(xmax)) {
...
if (!nexacro.ChartBasicSeriesControl) {
	...
	_pChartBasicSeriesControl.shadowstyle = null;
	_pChartBasicSeriesControl.barradius = 0;
	...
	_pChartBasicSeriesControl.autogradation = "none";
	...
	_pChartBasicSeriesControl.set_shadowstyle = function (val) {
		if (val == "") {
			val = null;
		}
	...
	_pChartBasicSeriesControl.on_apply_shadowstyle = function (shadowstyle) {
		this._redrawSeries = false;
		this._applyPropertySeries("Line", "shadowstyle", shadowstyle);
	...
	_pChartBasicSeriesControl.set_autogradation = function (val) {
		if (!val || val == "") {
			val = "none";
		}
	...
	_pChartBasicSeriesControl.on_apply_autogradation = function (autogradation) {
		this._redrawSeries = false;
	};
	...
	_pChartBasicSeriesControl.on_apply_linevisible = function (linevisible) {
		var barvisible = this.barvisible, pointvisible = this.pointvisible, stacktype = this._stacktype || this._chart.stacktype;
	...
	_pChartBasicSeriesControl.set_barradius = function (val) {
		val = parseInt(val);
		this.barradius = val;
		if (val > 0) {
	...
	_pChartBasicSeriesControl.on_apply_barradius = function (barradius) {
		this._redrawSeries = false;
	...
	_pChartBasicSeriesControl._drawSeriesBars = function () {
		...
		var isTagetStackTotal = false;
		if (this._chart.stacktotalvisible == true && stacktype == "normal") {
			isTagetStackTotal = true;
		}
		var stackbargroup = this.stackbargroup;
		if (!stackbargroup) {
			stackbargroup = "_default_";
		}
		var objPosition;
		if (!this._chart._stackbargrouplist) {
			this._chart._stackbargrouplist = [];
		}
		var stackbarlist = this._chart._stackbargrouplist;
		if (!stackbarlist[stackbargroup]) {
			stackbarlist[stackbargroup] = [];
		}
		var stackbargrouplist = stackbarlist[stackbargroup];
		if (effect && effect.isloadanimation) {
			isTagetStackTotal = false;
		}
		...
		for (var i = 0; i < pLeng; i += ps) {
			...
			if (isselectitem) {
				objPosition = this._drawBar(x, y, b, barLeft, barRight, this._xaxis, this._yaxis, selectbarlinestyle, selectbarfillstyle, selectbaropacity, index);
			}
			else {
				if (barfillstyle instanceof Array) {
					var barfillstyle_sv = barfillstyle[index % barfillstyle.length];
					objPosition = this._drawBar(x, y, b, barLeft, barRight, this._xaxis, this._yaxis, barlinestyle, barfillstyle_sv, baropacity, index);
				}
				else {
					objPosition = this._drawBar(x, y, b, barLeft, barRight, this._xaxis, this._yaxis, barlinestyle, barfillstyle, baropacity, index);
				}
			}
			if (isTagetStackTotal) {
				if (stackbargrouplist[index]) {
					if (rotateaxis) {
						stackbargrouplist[index].w += objPosition.w;
						if (stackbargrouplist[index].l > objPosition.l) {
							stackbargrouplist[index].l = objPosition.l;
						}
						else {
							stackbargrouplist[index].r = objPosition.r;
						}
					}
					else {
						stackbargrouplist[index].h += objPosition.h;
						if (stackbargrouplist[index].t > objPosition.t) {
							stackbargrouplist[index].t = objPosition.t;
						}
						else {
							stackbargrouplist[index].b = objPosition.b;
						}
					}
				}
				else {
	...
	_pChartBasicSeriesControl._drawBar = function (x, y, b, barLeft, barRight, axisx, axisy, barlinestyle, barfillstyle, baropacity, index, item) {
		...
		if (barfillstyle instanceof Array) {
			...
			if (barfillstyle && this.autogradation != "none") {
				var bTarget = false;
				if (barfillstyle instanceof nexacro._BackgroundObject) {
					if (!barfillstyle.gradient) {
						bTarget = true;
						barfillstyle = barfillstyle.value;
					}
				}
				else if (barfillstyle) {
					bTarget = (barfillstyle.indexOf("gradient") < 0) ? true : false;
				}
				if (bTarget) {
					var anglePos = (this.parent.rotateaxis == true) ? "bottom" : "right";
					var darkcolor1;
					if (this.autogradation == "center") {
						var lightcolor1 = this._shadeColor(barfillstyle, -30);
						var lightcolor2 = this._shadeColor(barfillstyle, -50);
						darkcolor1 = this._shadeColor(barfillstyle, 20);
						sv_barfillstyle = "linear-gradient(to " + anglePos + ","
							 + darkcolor1 + " 0%,"
							 + barfillstyle + " 25%,"
							 + lightcolor1 + " 40%,"
							 + lightcolor2 + " 50%,"
						...
						var darkcolor0 = this._shadeColor(barfillstyle, 5);
						darkcolor1 = this._shadeColor(barfillstyle, 10);
						var darkcolor2 = this._shadeColor(barfillstyle, 25);
						var darkcolor3 = this._shadeColor(barfillstyle, 40);
						sv_barfillstyle = "linear-gradient(to " + anglePos + ","
							 + darkcolor3 + " 0%,"
							 + darkcolor1 + " 10%,"
							 + darkcolor0 + " 25%,"
		...
		if (rect && !item) {
			...
			if (itemtextvisible) {
				var isEffect = (effect && effect.isloadanimation);
				if (!isEffect) {
					if (chart._bargrouping && (!this.stacktype || this.stacktype == "none") && !item) {
						if (!chart._groupItemTextList) {
							chart._groupItemTextList = [];
						}
						var list = chart._groupItemTextList[this._configIndex];
						if (!list) {
							list = chart._groupItemTextList[this._configIndex] = [];
						}
						var pushdata = {
							"left" : left, 
							"right" : right, 
	...
	_pChartBasicSeriesControl._drawStackBar = function (stackgroupname, index, left, top, width, height, right, bottom, value) {
		var rect, seriesId, chart = this._chart, seriesGroup = chart._seriesGroup;
		if (!seriesGroup) {
			return;
		}
		...
		if (this._chart._stacktotalshadowstyle) {
			rect.set_shadow(this._chart._stacktotalshadowstyle);
		}
		if (this._chart._stacktotallinestyle) {
			rect.set_strokepen(this._chart._stacktotallinestyle || "1px solid #dddddd");
		}
		rect._index = index;
		rect._value = value;

		this._drawBarItemText(left, right, bottom, top, width, height, rect, stackgroupname);
	};
	...
	_pChartBasicSeriesControl._drawPoint = function (x, y, radius, axisx, axisy, pointshape, _pointshape, pointlinestyle, pointfillstyle, pointopacity, index, item) {
		var chart = this._chart, point, cx, cy, points = [], seriesId, shiftPointX = 0, shiftPointY = 0, seriesGroup = chart._seriesGroup, rotateaxis = chart.rotateaxis, tmp, highlightGroup = chart._highlightGroup, effect = this._chart_aniframe_obj, tickstartgap;
		...
		if (item) {
			...
			if (!highlightitem) {
				if (pointshape == "square" || pointshape == "diamond" || pointshape == "triangle" || pointshape == "cross") {
					...
					point = new nexacro.ChartGraphicsEllipse(cx, cy, radius, radius);
				}
	...
	_pChartBasicSeriesControl._drawLine = function (datapoints, xoffset, yoffset, axisx, axisy, linestyle, lineopacity, Ishighlight) {
		var points = datapoints.points.slice(0), ps = datapoints.pointsize, prevx = null, prevy = null, path, shiftPointX = 0, startPointX, endPointX, shiftPointY = 0, startPointY, endPointY, tickstartgap, rotateaxis = this._chart.rotateaxis, seriesGroup = this._chart._seriesGroup, bCreate = false, effect = this._chart_aniframe_obj;
		...
		if (this.shadowstyle) {
			path.set_shadow(this.shadowstyle);
		}
		path.set_strokepen(linestyle);

		if (!nexacro._ChartGraphicsLib.isEmpty(lineopacity)) {
	...
	_pChartBasicSeriesControl._drawArea = function (datapoints, axisx, axisy, lineareafillstyle, lineareaopacity) {
		var points = datapoints.points.slice(0), ps = datapoints.pointsize, i = 0, areaOpen = false, ypos = 1, xpos = 0, segmentStart = 0, segmentEnd = 0, area, tickstartgap, seriesGroup = this._chart._seriesGroup, bCreate = false, effect = this._chart_aniframe_obj, rotateaxis = this._chart.rotateaxis;
		...
		if (!nexacro._ChartGraphicsLib.isEmpty(lineareaopacity)) {
	...
	_pChartBasicSeriesControl._drawCurveArea = function (points, vpoints, lineareafillstyle, lineareaopacity) {
		var boardRect = this._chart._boardRect, borderHeight = this._chart._boardBorderHeight, boardHeight = 0, boardWidth = 0, zeroBase = 0, xaxis = this._xaxis, yaxis = this._yaxis, rotateaxis = this._chart.rotateaxis, seriesGroup = this._chart._seriesGroup, bCreate = false, i, x1, y1, cx, cy, x2, y2, type, area;
	...
	_pChartBasicSeriesControl._drawBarItemText = function (left, right, bottom, top, width, height, item, stackgroupname, index, uservalue) {
		var seriesGroup = this._chart._seriesGroup;

		var itemText, itemtextPosition, itemtextGap;
		if (stackgroupname !== undefined && stackgroupname !== null) {
			var itemtextmask = this.itemtextmask, itemtextfont = this.itemtextfont, itemtextcolor = this._chart._stacktotaltextcolor, itemtextwidth = this.itemtextwidth, chart = this._chart;
			var index2 = item._index;
			var uservalue2 = item._value;
			...
			if (!itemText) {
				...
				this._itemtextlist.push(textId);
			}
			itemText.set_text(text);
			itemText.set_font(itemtextfont ? itemtextfont.value || itemtextfont : "12pt Verdana");
			itemText.set_color(itemtextcolor ? itemtextcolor.value || itemtextcolor : "#000000");
			itemText.index = item.index;
			itemText.value = uservalue2;
			if (itemtextwidth > 0) {
				itemText.set_width(itemtextwidth);
				itemText.set_wordWrap(true);
			}
			else {
				itemText.set_wordWrap(false);
			}
			...
			itemtextPosition = this.baritemtextposition;
			itemtextGap = this.baritemtextgap;
		}
	...
	_pChartBasicSeriesControl._drawPointItemText = function (cx, cy, item) {
		...
		if (!nexacro._isNull(itemText)) {
			var itemtextPosition = this.pointitemtextposition, itemtextGap = this.pointitemtextgap;
	...
	_pChartBasicSeriesControl._drawLineItemText = function (datapoints, axisx, axisy, item) {
		...
		for (var i = 0; i < points.length; i += ps) {
			var itemindex = points[i], preitemindex = points[i + ps], value = points[i + 1], linetype = this.linetype, itemText;
			...
			itemText = this._createSeriesItemText(item);
			if (!nexacro._isNull(itemText)) {
				var cx = 0, cy = 0, width = this._lineborderwidth, seriesGroup = this._chart._seriesGroup;
	...
	_pChartBasicSeriesControl._showBarHighlight = function (item) {
		...
		if (!barHighlight) {
			var points = item._points, index = item.index, highlightbarlinestyle = this.highlightbarlinestyle || "1px solid " + this._highlightcolor, highlightbarfillstyle = this.highlightbarfillstyle || this._highlightcolor, highlightbaropacity = this.highlightbaropacity, barLeft, barRight, barwidth;
	...
	_pChartBasicSeriesControl._showPointHighlight = function (item) {
		...
		if (!pointHighlight) {
			var points = item._points, index = item.index, highlightpointlinestyle = this.highlightpointlinestyle || "1px solid " + this._highlightcolor, highlightpointfillstyle = this.highlightpointfillstyle || this._highlightcolor, highlightpointopacity = this.highlightpointopacity, highlightpointsize = this.highlightpointsize;
	...
	_pChartBasicSeriesControl._hidePointHighlight = function (item) {
		...
		var pointHighlight = item._pointHighlight;
		if (pointHighlight && !nexacro._ChartGraphicsLib.isEmpty(pointHighlight.parent)) {
			this._chart._chageGroupObject(this._chart._seriesGroup, this._chart._highlightGroup, this._itemtextlist, true);
	...
	_pChartBasicSeriesControl._hideLineHighlight = function (item) {
		...
		var lineHighlight = item._lineHighlight;
		if (lineHighlight && !nexacro._ChartGraphicsLib.isEmpty(lineHighlight.parent)) {
			this._chart._chageGroupObject(this._chart._seriesGroup, this._chart._highlightGroup, this._itemtextlist, true);
	...
	_pChartBasicSeriesControl._setColor = function (colorset) {
		...
		if (changedColorset) {
			var barvisible = this.barvisible, pointvisible = this.pointvisible, linevisible = this.linevisible, barlinestyle, barfillstyle, linestyle, lineareafillstyle, pointlinestyle, pointfillstyle;
