...
if (!nexacro._ShadowObjectNew) {
	nexacro._ShadowObjectNew = function (v) {
		this.value = "";
		this.rtlvalue = "";
		this.offsetx = 0;
		this.offsety = 0;
	...
	var _pShadowObjectNew = nexacro._createPrototype(nexacro.Object, nexacro._ShadowObjectNew);
	nexacro._ShadowObjectNew.prototype = _pShadowObjectNew;
	_pShadowObjectNew._type_name = "ShadowObject";

	_pShadowObjectNew.valueOf = function () {
		return this.value;
	};
	_pShadowObjectNew.getOffsetX = function () {
		if (!this.offsetx || this.offsetx == "") {
			return 0;
		}
		return parseInt(this.offsetx.replace("px", ""));
	};
	_pShadowObjectNew.getOffsetY = function () {
		if (!this.offsety || this.offsety == "") {
			return 0;
		}
		return parseInt(this.offsety.replace("px", ""));
	};
	_pShadowObjectNew.getBlur = function () {
		if (!this.blur || this.blur == "") {
			return 0;
		}
		return parseInt(this.blur.replace("px", ""));
	};
	_pShadowObjectNew.getColor = function () {
		return nexacro.ColorObject(this.color);
	};
	_pShadowObjectNew.toString = function () {
		return this.value;
	};

	nexacro._ShadowObjectNew_caches = {
	};
	nexacro.ShadowObjectNew = function (val) {
		if (val instanceof nexacro._ShadowObjectNew) {
			return val;
		}
		var obj = nexacro._ShadowObjectNew_caches[val];
		if (obj) {
			return obj;
		}
		obj = new nexacro._ShadowObjectNew(val);
		nexacro._ShadowObjectNew_caches[val] = obj;
		return obj;
	};
}
...
if (!nexacro._ChartBase) {
	...
	nexacro.ChartDrawEndEventInfo = function (obj, id) {
		this.id = this.eventid = id || "ondrawend";
		this.fromobject = obj;
	};

	var _pChartDrawEndEventInfo = nexacro._createPrototype(nexacro.Event, nexacro.ChartDrawEndEventInfo);
	nexacro.ChartDrawEndEventInfo.prototype = _pChartDrawEndEventInfo;
	_pChartDrawEndEventInfo._type_name = "ChartDrawEndEventInfo";
	...
	_pChartBase.on_apply_locale = function (val) {
		...
		nexacro._ChartGraphicsLibArray.forEach(this.seriesset, function (series, index) {
			if (series) {
				series.on_apply_locale();
			}
		}, this);

		if (charttype == "BasicChart" || charttype == "BubbleChart" || charttype == "GaugeChart" || charttype == "FloatChart") {
			nexacro._ChartGraphicsLibArray.forEach(this._axes, function (axis, index) {
				if (axis) {
					axis._locale = val;
		...
		else if (charttype == "RadarChart" || charttype == "RoseChart") {
			...
			if (valueaxes) {
				nexacro._ChartGraphicsLibArray.forEach(this.valueaxes, function (axis, index) {
					if (axis) {
						axis._locale = val;
						axis.on_apply_locale();
					}
	...
	_pChartBase.set_colorset = function (val) {
		if (nexacro._ChartGraphicsLib.isEmpty(val)) {
			val = "color10";
		}
	...
	_pChartBase.on_apply_colorset = function (colorset) {
		var index = nexacro._ChartGraphicsLibArray.indexOf(this._colorCategory, colorset);
		var index1 = nexacro._ChartGraphicsLibArray.indexOf(this._colorCategory, "colorRed10");
		var index2 = nexacro._ChartGraphicsLibArray.indexOf(this._colorCategory, "colorPurple10");
	...
	_pChartBase.removeUserControl = function (id) {
		var group = this._seriesGroup;
		var itemId = "UserControlItem_" + id;
		var item = group.getObjectByID(itemId);
		if (item) {
			group.removeChild(item);
			item.destroy();
		}
	};
	_pChartBase.userControlDraw = function (item) {
		this._graphicsControl._repaint = true;
		this._graphicsControl.draw();
	};
	_pChartBase.getCanvasContext = function () {
		return this._graphicsControl._layer._canvas;
	};

	_pChartBase.addUserControl = function (id, type) {
		if (!this._seriesGroup || !id || !type) {
			return;
		}
		...
		if (!item) {
			switch (type) {
				case "rect":
					item = new nexacro.ChartGraphicsRect(0, 0, 0, 0);
					break;
				case "arc":
					item = new nexacro.ChartGraphicsEllipse(0, 0, 0, 0);
					break;
				case "path":
					item = new nexacro.ChartGraphicsPath();
					break;
				case "text":
					item = new nexacro.ChartGraphicsText(0, 0);
	...
	_pChartBase.setCategoryaxis = function (val) {
		var re = false, cont = {
	...
	_pChartBase.getBorderPosInfo = function () {
		var board = this.board;
		if (board) {
			return {
				"l" : this._boardRectLeft, 
				"t" : this._boardRectTop, 
				"w" : this._boardWidth, 
				"h" : this._boardHeight
			};
		}
	...
	_pChartBase.getSeriesPointPos = function (seriesIdx, dataIdx) {
		var series = this.seriesset[seriesIdx];
		if (!series) {
			return;
		}
		var items = series._seriesitems;
		var returnData = null;
		if (items && items[dataIdx]) {
			var item = items[dataIdx];
			if (item._slice && item.pathData) {
				var boardborderWidth = this._boardBorderWidth ? this._boardBorderWidth : 0;
				var boardborderHeight = this._boardBorderHeight ? this._boardBorderHeight : 0;
				var maxRadius = Math.min((this._boardWidth - (boardborderWidth * 2)), (this._boardHeight - (boardborderHeight * 2))) / 2;
				if (this.valueaxes) {
					maxRadius -= (this.valueaxes[0] ? this.valueaxes[0]._allaxisgapsize : 0);
				}
				returnData = {
					"startangle" : item.startAngle, 
					"endangle" : item.endAngle, 
					"rstartangle" : item.rStartAngle, 
					"rendangle" : item.rEndAngle, 
					"angle" : item.angle, 
					"bound" : item._slice._bounds, 
					"centerx" : this._centerLeft, 
					"centery" : this._centerTop, 
					"posinfo" : item._posinfo, 
					"radius" : maxRadius
				};
			}
			else {
	...
	_pChartBase.getAxisPointPos = function (type, value, axisindex, tickindex) {
		var axis;
		if (type == "category") {
			axis = this.categoryaxis;
		}
		else {
			axisindex = !axisindex ? 0 : axisindex;
			axis = this.valueaxes[axisindex];
		}
		if (!axis) {
			return;
		}

		var axisticks = axis._ticks;
		var checkXI = -1;
		if (tickindex != undefined && tickindex != null) {
			checkXI = tickindex;
		}
		else {
			for (var xi = 0; xi < axisticks.length; xi++) {
				if (isNaN(value)) {
					if (axisticks[xi].label == value) {
						checkXI = xi;
						break;
					}
				}
				else {
					if (axisticks[xi].v == value) {
						checkXI = xi;
						break;
					}
				}
		...
		if (checkXI >= 0) {
			var elem = axisticks[checkXI].boardlineElement;
			return {
				"x" : elem.x1 + axis._tickstartgap, 
				"y" : elem.y2, 
				"point" : elem._point
			};
		}
	...
	_pChartBase.updateContents = function (contents) {
		...
		var capitalize = nexacro._ChartGraphicsLibString.capitalize;
		var capNm;
		var value;
		var setter;
		var subControl;
		...
		nexacro._ChartGraphicsLibArray.forEach(this._createdOrder, function (name, index) {
			...
			if (nexacro._ChartGraphicsLib.isObject(value)) {
				...
				nexacro._ChartGraphicsLibArray.forEach(value, function (o, idx) {
					if (name == "valueaxes") {
	...
	_pChartBase.getSeriesByID = function (id) {
		var index = nexacro._ChartGraphicsLibArray.indexOfProp(this.seriesset, "id", id);
		if (index > -1) {
	...
	_pChartBase._clearMouseActionSeriesItem = function () {
		if (this._overSeriesItem) {
			var tooltip = this.tooltip, series = this._overSeriesItem._series, seriesItemType = this._overSeriesItem._type, chartName;
			...
			else if (overitemid.search("RoseItem") >= 0) {
				oversearchrose = true;
			}
			...
			if (seriesItemType) {
				if (seriesItemType == "Rect" && oversearchbar) {
				...
				else if (oversearchrose || oversearchpoint || oversearchline || oversearcharea || oversearchetc || oversearchgauge) {
					...
					else if (chartName == "RoseChart") {
						if (oversearchrose) {
							series._hideRoseHighlight(this._overSeriesItem);
						}
					}
	...
	_pChartBase.deleteSeries = function (val) {
		...
		if (typeof (val) == "string") {
			...
			if (series) {
				index = nexacro._ChartGraphicsLibArray.indexOf(this.seriesset, series);
				if (index > -1) {
	...
	_pChartBase._drawEndEvent = function () {
		if (this.ondrawend && this.ondrawend._has_handlers) {
			var evt = new nexacro.ChartDrawEndEventInfo(this);
			return this.ondrawend._fireUserEvent(this, evt);
		}
	};
	...
	_pChartBase.getHittestRefComponent = function (canvasX, canvasY) {
		var targets, index = undefined, value, itemindex = undefined, itemobject = undefined, len = 0, refobject = {
			"object" : null, 
			"eventid" : "onclick", 
			"index" : undefined, 
			"value" : undefined, 
				...
				if (targets[i]["_series"]) {
					if (targets[i]["_legend"]) {
						...
						if (this._type_name == "PieChart") {
							...
							if (this.gaugetype == "circular") {
								...
								index = nexacro._ChartGraphicsLibArray.indexOfProp(this.seriesset, "id", targets[i]["_series"].id);
							}
						}
						else {
							index = nexacro._ChartGraphicsLibArray.indexOfProp(this.seriesset, "id", targets[i]["_series"].id);
						...
						if (this._type_name == "PieChart") {
							...
							if (overitemid.search("SeriesItemText_") >= 0) {
								...
								else if (this._type_name == "RadarChart" || this._type_name == "RoseChart") {
									s = targets[i]["_series"];
					...
					if (targets[i]["_axis"]._type == "categoryAxis") {
						...
						if (this._type_name == "RadarChart" || this._type_name == "RoseChart") {
							...
							itemindex = nexacro._ChartGraphicsLibArray.indexOfProp(this.valueaxes, "id", targets[i]["_axis"].id);
						}
					}
					break;
				}
							...
							if (seriesItemType == "Rect" && searchbar) {
								...
								if (chartName == "BasicChart") {
									...
									if (series.highlightvisible) {
										series._showRoseHighlight(seriesItem);
									}
								}
								else if (chartName == "GaugeChart" || chartName == "FloatChart") {
									if (series.visible && series.highlightbarvisible) {
	...
	_pChartBase._draw = function () {
		...
		if (this._loadanimation && this._binddataset.rowcount == 0) {
		...
		else if (this._loadanimation == false && this._isanimationloading == false) {
			this._drawEndEvent();
		}
	};
	...
	_pChartBase._setDatasetEventHandlers = function (ds) {
		...
		if (length > 1) {
			nexacro._ChartGraphicsLibArray.move(uHandler, length - 1, 0);
		}
			...
			switch (e.reason) {
				case 12:
					...
					for (; i >= row; i--) {
						...
						if (this._type_name == "BubbleChart" || this._type_name == "FloatChart") {
	...
	_pChartBase._createObjectItem = function (ds, row, key) {
		...
		for (var i = 0, collen = ds.getColCount(); i < collen; i++) {
			...
			if (nexacro._ChartGraphicsLib.isEmpty(value)) {
				value = null;
			}
			data[name] = value;
		}
	...
	_pChartBase._deleteSeries = function (series, index) {
		...
		nexacro._ChartGraphicsLibArray.removeAt(this.seriesset, index);
		...
		nexacro._ChartGraphicsLibArray.forEach(this.seriesset, function (obj, idx) {
	...
	_pChartBase._createSubControl = function (contents) {
		...
		nexacro._ChartGraphicsLibArray.forEach(this._createdOrder, function (name, index) {
			...
			if ((charttype == "GaugeChart" || charttype == "RadarChart" || charttype == "BasicChart" || charttype == "RoseChart" || charttype == "FloatChart") && capNm == "Categoryaxis") {
		...
		if ((charttype == "RadarChart" || charttype == "BasicChart" || charttype == "RoseChart" || charttype == "FloatChart") && !createdCategoryaxis) {
			var axis = this._createCategoryaxis();
			axis._afterSetProperties();
		}

		if (charttype == "RadarChart" || charttype == "RoseChart" || charttype == "GaugeChart") {
	...
	_pChartBase._createBoard = function (o, id) {
		if (!this.board) {
			...
			if (!this._seriesGroup) {
				this._seriesGroup = new nexacro.ChartGraphicsGroup();
				this._seriesGroup.set_id("ChartSeriesGroup");
				this._graphicsControl.addChild(this._seriesGroup);
			}

			if (!this._highlightGroup) {
				this._highlightGroup = new nexacro.ChartGraphicsGroup();
	...
	_pChartBase._mergeContents = function () {
		...
		if (typeof target !== "object" && !nexacro._ChartGraphicsLib.isFunction(target)) {
			target = {
			};
		}
		...
		for (; idx < length; idx++) {
			if ((obj = arguments[idx]) != null) {
				for (prop in obj) {
					...
					if (isDeep && objVal && (nexacro._ChartGraphicsLib.isObject(objVal) || (objValIsArray = nexacro._ChartGraphicsLib.isArray(objVal)))) {
						if (objValIsArray) {
							objValIsArray = false;
							clone = targetVal && nexacro._ChartGraphicsLib.isArray(targetVal) ? targetVal : [];
						}
						else {
							clone = targetVal && nexacro._ChartGraphicsLib.isObject(targetVal) ? targetVal : {
							};
						}
	...
	_pChartBase._arrangeTitle = function () {
		...
		if (title && title.visible) {
			var boardRect = this._boardRect, clientWidth = this._getClientWidth(), clientHeight = this._getClientHeight(), titlespacing = this._titlespacing || this._nullspacing, titleRect, titleHeight = 0, titleWidth = 0, titleLeft = 0, titleTop = 0, halign = title._halign || "center", valign = title._valign || "top", position = title._position || "top";
			var xSize = 0;
	...
	_pChartBase._drawStackTotal = function () {
		var seriesset = this.seriesset;
		var stacklist = this._stackbargrouplist;
		var listone, listoneTmp, listitem;
		var nLeng = 0;
		var series = seriesset[0];
		for (listone in stacklist) {
			listitem = stacklist[listone];
			nLeng = listitem.length;
			for (var j = 0; j < nLeng; j++) {
				listoneTmp = listone;
				if (!listoneTmp || listoneTmp == "") {
					listoneTmp = "_default_";
				}
				series._drawStackBar(listoneTmp, j, listitem[j].l, listitem[j].t, listitem[j].w, listitem[j].h, listitem[j].r, listitem[j].b, listitem[j].v);
			}
		}
		if (stacklist) {
			delete this._stackbargrouplist;
			this._stackbargrouplist = null;
		}

		var grpTextList = this._groupItemTextList;
		var data, seriesItem;
		if (grpTextList && grpTextList.length > 0) {
			for (var z = 0; z < grpTextList.length; z++) {
				if (!grpTextList[z] || grpTextList[z].length == 0) {
					continue;
				}
				seriesItem = grpTextList[z];
				for (var zs = 0; zs < seriesItem.length; zs++) {
					data = seriesItem[zs];
					seriesset[z]._drawBarItemText(data.left, data.right, data.bottom, data.top, data.width, data.height, data.rect);
				}
			}
		}
		if (grpTextList) {
			delete this._groupItemTextList;
			this._groupItemTextList = null;
		}
	};
	...
	_pChartBase._setProperties = function (conts, subControl) {
		...
		nexacro._ChartGraphicsLibObject.Each(conts, function (name, val, object) {
			setter = "set_" + name;
			if (this[setter]) {
				this[setter](val);
			}
...
if (!nexacro._AxisChartBase) {
	...
	_pAxisChartBase.setValueaxis = function (id, contents) {
		...
		if (valueaxisId) {
			...
			valueaxisIndex = nexacro._ChartGraphicsLibArray.indexOf(this.valueaxes, valueaxis);
		}
		...
		if (series) {
			...
			if (typeof (contents) == "object") {
				if (!nexacro._ChartGraphicsLib.isEmpty(contents)) {
					nexacro._ChartGraphicsLibObject.Each(contents, function (name, val, object) {
						if (name == "id") {
							if (charttype == "GaugeChart" || charttype == "PieChart") {
								seriesId = contents.id;
							}
							else {
	...
	_pAxisChartBase._appendValueaxis = function (contents) {
		...
		}, valueaxesLeng = -1, charttype = this._type_name, seriesset = this.seriesset, s1, s2, linevisible1 = false, location, i, linevisible2 = false;
	...
	_pAxisChartBase._initAxes = function () {
		nexacro._ChartGraphicsLibArray.Each(this._axes, function (axis, i) {
			if (axis) {
				axis._initData();
			}
		});
	...
	_pAxisChartBase._setDatapoint = function () {
		var seriesset = this.seriesset, s, pointshape, xaxis, yaxis;
	...
	_pAxisChartBase._arrangeHrangebar = function () {
		...
		if (hrangebar && hrangebar._group.visible) {
			var boardRect = this._boardRect, hrangebarspacing = this._hrangebarspacing || this._nullspacing, hrangebarRect, hrangebarRectHeight = 0, hrangebarHeight = 0, hrangebarLeft = 0, hrangebarTop = 0, boardBorderSize = this._boardBorderSize, boardBorderWidth = this._boardBorderWidth, boardspacing = this._boardspacing || this._nullspacing;
	...
	_pAxisChartBase._arrangeVrangebar = function () {
		...
		if (vrangebar && vrangebar._group.visible) {
			var boardRect = this._boardRect, vrangebarspacing = this._vrangebarspacing || this._nullspacing, vrangebarRect, vrangebarRectWidth = 0, vrangebarWidth = 0, vrangebarLeft = 0, vrangebarTop = 0, boardBorderWidth = this._boardBorderWidth, boardspacing = this._boardspacing || this._nullspacing;
	...
	_pAxisChartBase._arrangeRangebarBoardRect = function () {
		var vrangebar = this.vrangebar;
		var hrangebar = this.hrangebar;
		var boardRect = this._boardRect, vrangebarspacing = this._vrangebarspacing || this._nullspacing, hrangebarspacing = this._hrangebarspacing || this._nullspacing, boardBorderSize = this._boardBorderSize, boardBorderWidth = this._boardBorderWidth, boardspacing = this._boardspacing || this._nullspacing, hrangebarRect, hrangebarRectHeight = 0, hrangebarHeight = 0, hrangebarLeft = 0, hrangebarTop = 0, vrangebarRect, vrangebarRectWidth = 0, vrangebarWidth = 0, vrangebarLeft = 0, vrangebarTop = 0;
	...
	_pAxisChartBase._arrangeAxes = function () {
		...
		for (var i = 0; i < axes.length; i++) {
			...
			if (charttype == "BasicChart" || charttype == "FloatChart") {
	...
	_pAxisChartBase._deleteAxis = function (axis, isCategory, index) {
		...
		if (this._axes.length > 0) {
			nexacro._ChartGraphicsLibArray.remove(this._axes, axis);
		}

		var location = axis._location;
		if (location == "left" || location == "right") {
			if (this._yaxes.length > 0) {
				nexacro._ChartGraphicsLibArray.remove(this._yaxes, axis);
			}
		}
		else if (location == "bottom" || location == "top") {
			if (this._xaxes.length > 0) {
				nexacro._ChartGraphicsLibArray.remove(this._xaxes, axis);
			}
		}
...
if (!nexacro.ChartAxisControl) {
	...
	_pChartAxisControl._tickintervalcategory = 0;
	...
	_pChartAxisControl.set_tickinterval = function (val) {
		...
		if (val !== "" && val != null) {
			...
			if (this.tickinterval !== val) {
				...
				if (this._type == "categoryAxis") {
					this._tickintervalcategory = val;
				}
		...
		if (parent_clear) {
			...
			if (this.parent) {
				if (this.parent._axes) {
					nexacro._ChartGraphicsLibArray.remove(this.parent._axes, this);
				}
				...
				if (location == "left" || location == "right") {
					if (this.parent._yaxes) {
						nexacro._ChartGraphicsLibArray.remove(this.parent._yaxes, this);
					}
				}
				else if (location == "bottom" || location == "top") {
					if (this.parent._xaxes) {
						nexacro._ChartGraphicsLibArray.remove(this.parent._xaxes, this);
					}
				}
			}
		}
			...
			for (var i = 0; i < this._ticks.length; i++) {
				...
				if (tick) {
					...
					if (tickline) {
						tickline.destroy();
						if (tick.lineElement) {
							delete tick.lineElement;
							tick.lineElement = null;
						}
					}
					...
					if (tickboardline) {
						...
						if (tick.boardlineElement) {
							delete tick.boardlineElement;
							tick.boardlineElement = null;
						}
					}
					this._ticks[i] = null;
				}
	...
	_pChartAxisControl._setCategoriesData = function (datapoints) {
		var points = datapoints.points, ps = datapoints.pointsize, format = datapoints.format, formatColumn = this._direction, categories, index = this._getNextIndex(categories);
	...
	_pChartAxisControl._preGenerateTimeTick = function () {
		...
		var delta = (max - min) / tickscount, dec = -Math.floor(Math.log(delta) / Math.LN10), magn = Math.pow(10, -dec), norm = delta / magn, tickinterval;
		if (norm < 1.5) {
			tickinterval = 1;
		}
		else if (norm < 3) {
	...
	_pChartAxisControl._setupTickGeneration = function (width, height) {
		...
		var delta = (this._max - this._min) / ticks, dec = -Math.floor(Math.log(delta) / Math.LN10), magn = Math.pow(10, -dec), norm = delta / magn, tickinterval;
	...
	_pChartAxisControl._setTicks = function (istotal) {
		...
		var subTick = this.smalltickcount;
		if (!subTick) {
			subTick = 0;
		}
		var subtickvaluegap = 0;
		var vv = 0;
		if (subTick > 1 && tickLength > 1) {
			var v0 = tickGen[0];
			var v1 = tickGen[1];
			if (typeof v0 == "object") {
				v0 = tickGen[0];
		...
		for (var i = 0; i < tickLength; i++) {
			...
			if (!isNaN(v)) {
				...
				if (subTick > 1 && i < (tickLength - 1)) {
					vv = v;
					for (var j = 1; j < subTick; j++) {
						vv += subtickvaluegap;
						this._ticks.push({
							v : vv, 
							label : label, 
	...
	_pChartAxisControl._setPixedLabelHeight = function (clientWidth, clientHeight) {
		var ticklinestyle = this._ticklinestyle, boardlinestyle = this._boardlinestyle, maxlabelheight = 0, maxlabelwidth = 0;

		var virtual_tickGroup = new nexacro.ChartGraphicsGroup();
		var virtual_boardLineGroup = new nexacro.ChartGraphicsGroup();
		var virtual_labelGroup = new nexacro.ChartGraphicsGroup();
		...
		for (var i = 0; i < ticks.length; i++) {
			...
			value = tick.v;
			if (nexacro._ChartGraphicsLib.isEmpty(tick.label) || value < this._datamin || value > this._datamax) {
				continue;
			}
	...
	_pChartAxisControl._setData = function (clientWidth, clientHeight) {
		var changedTicks = this._changedTicks, ticklinestyle = this._ticklinestyle, boardlinestyle = this._boardlinestyle, pixedlabelheight = 0, pixedlabelwidth = 0;
	...
	_pChartAxisControl._createTitle = function () {
		if (!this._title) {
			this._title = new nexacro.ChartGraphicsText(0, 0);
	...
	_pChartAxisControl._measureAxisRect = function () {
		...
		var axislinestyle = this._axislinestyle, axislinewidth = 0, size = 0, ticksize, ticklinestyle = this._ticklinestyle, ticklineWidth = 0, labelgap = this._labelgap, labelWidth = this._labelWidth, labelHeight = this._labelHeight, titlegap = this.titlegap || 0, titlerotate = this._titlerotate;
	...
	_pChartAxisControl._changeContentsProperty = function (nm, newVal, oldVal) {
		...
		if (type == "categoryAxis") {
			...
			idx = nexacro._ChartGraphicsLibArray.indexOf(valueaxes, this);
...
if (!nexacro.ChartRangebarControl) {
	...
	_pChartRangebarControl._createRangebarRect = function () {
		this._rangebarRect = new nexacro.ChartGraphicsRect(0, 0, 0, 0);
	...
	_pChartRangebarControl._createTrackbarRect = function () {
		this._trackbarRect = new nexacro.ChartGraphicsRect(0, 0, 0, 0);
	...
	_pChartRangebarControl._createDecImage = function () {
		this._decImage = new nexacro.ChartGraphicsImage(0, 0);
	...
	_pChartRangebarControl._createIncImage = function () {
		this._incImage = new nexacro.ChartGraphicsImage(0, 0);
	...
	_pChartRangebarControl._dragObject = function (obj, x, y) {
		var pThis = this, chart = this.parent, trackbarRect = this._trackbarRect, origin = [x, y], dragged = 0, distance = [0, 0];
		...
		return {
			moved : function (button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY) {
				...
				if (!dragged && (dragged |= distance[0] | distance[1])) {
					...
					var start = 0, end = 0, startdata = pThis._startdata, enddata = pThis._enddata, cursor = obj._cursor, linestyle = pThis._linestyle, borderwidth = 0, trackbarpadding = pThis._trackbarpadding, paddingLeft = 0, paddingRight = 0, paddingTop = 0, paddingBottom = 0, canvasLeft = 0, canvasTop = 0, trackbarborderwidth = 0, trackbarlinestyle = pThis._trackbarlinestyle, trackbarminsize = pThis.trackbarminsize || 50, trackbargripsize = pThis.trackbargripsize || 10, boundW = 0, boundH = 0, datamax = pThis._axis._tickmax || pThis._axis._datamax, startpadding, endpadding, dataend = pThis.p2cScroll(datamax);
...
if (!nexacro.ChartSelectionControl) {
	...
	_pChartSelection._createSelection = function () {
		if (!this._selectionRect) {
			this._selectionRect = new nexacro.ChartGraphicsRect(-1, -1, 1, 1);
	...
	_pChartSelection._dragselectionObject = function (obj, x, y, canvasX, canvasY) {
		var pThis = this, chart = this.parent, selectionRect = this._selectionRect, origin = [x, y], dragged = 0, distance = [0, 0], i;
		var boardRect = null;
		if (chart) {
	...
	_pChartSelection._extractRange = function (axis, pos) {
		...
		var ticks = axis._ticks, axisvalue = null;
		if (ticks) {
	...
	_pChartSelection._getSelection = function () {
		...
		nexacro._ChartGraphicsLibArray.Each(this.parent._axes, function (axis, i) {
			if (axis) {
	...
	_pChartSelection._fireSelectedEvent = function () {
		...
		if (zoom) {
			...
			if (type.indexOf("x") > -1) {
				nexacro._ChartGraphicsLibObject.Each(ranges, function (axisname, object) {
...
if (!nexacro.ChartCrosshairControl) {
	...
	_pChartCrosshair._createCrosshairX = function () {
		if (!this._crosshairX) {
			this._crosshairX = new nexacro.ChartGraphicsLine();
	...
	_pChartCrosshair._createCrosshairY = function () {
		if (!this._crosshairY) {
			this._crosshairY = new nexacro.ChartGraphicsLine();
	...
	_pChartCrosshair._createXlineRect = function () {
		if (!this._xlinerect) {
			this._xlinerect = new nexacro.ChartGraphicsRect();
	...
	_pChartCrosshair._createYlineRect = function () {
		if (!this._ylinerect) {
			this._ylinerect = new nexacro.ChartGraphicsRect();
	...
	_pChartCrosshair._createXlineText = function () {
		if (!this._xlinetext) {
			this._xlinetext = new nexacro.ChartGraphicsText();
	...
	_pChartCrosshair._createYlineText = function () {
		if (!this._ylinetext) {
			this._ylinetext = new nexacro.ChartGraphicsText();
...
if (!nexacro._SeriesBase) {
	...
	_pSeriesBase._pointshapeObj = {
		square : function (x, y, radius, point) {
			...
			if (point) {
				...
				var rect = new nexacro.ChartGraphicsRect(x - size, y - size, size + size, size + size);
				return rect;
			}
		}, 
		...
		cross : function (x, y, radius, point) {
			...
			if (point) {
				...
				var paths = new nexacro.ChartGraphicsPaths();
	...
	_pSeriesBase._spliterColor = function (v, bObject) {
		var i;
		if (v && (typeof (v) == "string")) {
			var val = v.trim();
			if (val) {
				...
				for (i = 0; i < str.length; i++) {
					if (str[i] == "") {
						...
						if (bObject) {
							if (str[i] instanceof nexacro._BackgroundObject) {
								fillarr[cnt] = str[i];
							}
							else {
								fillarr[cnt] = nexacro.BackgroundObject(str[i], this);
							}
						}
						else {
							fillarr[cnt] = str[i];
	...
	_pSeriesBase._shadeColor = function (hueIn, pcIn) {
		if (hueIn == "transparent" || hueIn == "") {
			return hueIn;
		}
		if (nexacro._xreNamedColorList[hueIn]) {
			hueIn = nexacro._xreNamedColorList[hueIn];
		}
		if (hueIn.substring(0, 1) != "#") {
			return hueIn;
		}
		if (hueIn.length < 7) {
			return hueIn;
		...
		var red = parseInt(hueIn.substring(1, 3), 16);
		var green = parseInt(hueIn.substring(3, 5), 16);
		var blue = parseInt(hueIn.substring(5, 7), 16);
		var pc = parseInt(pcIn, 10);
		var max = 0;
		var dif = 0;
		...
		if (pc < 0) {
			...
			return "#" + ((truncate(red + dif)).toString(16)).padLeft(2, '0') + ((truncate(green + dif)).toString(16)).padLeft(2, '0') + ((truncate(blue + dif)).toString(16)).padLeft(2, '0');
		}
		else {
			if (green > max) {
				max = green;
	...
	_pSeriesBase.setSelectItem = function (index, bSelect_) {
		...
		if (nexacro._ChartGraphicsLib.isEmpty(index) || index < -1 || index > itemcnt || nexacro._ChartGraphicsLib.isEmpty(bSelect_)) {
			return false;
		}
		...
		if (parent_clear) {
			if (this._chart) {
				if (this._chart.seriesset) {
					...
					nexacro._ChartGraphicsLibArray.forEach(this._chart.seriesset, function (series, i) {
						if (series) {
					...
					nexacro._ChartGraphicsLibArray.removeAt(this._chart.seriesset, index);
				}
			}
		}
	...
	_pSeriesBase._createSeriesItemText = function (item) {
		...
		if (itemtext) {
			text = nexacro._getChartDisplaytText(index, itemtext, locale, itemtexttype, itemtextmask, chart, this, this);
		}
		...
		if (!nexacro._isNull(text)) {
			...
			if (charttype == "RadarChart" || charttype == "RoseChart") {
	...
	_pSeriesBase._setData = function () {
		...
		if (chart) {
			...
			if (binddataset) {
				if (charttype == "RoseChart" || charttype == "RadarChart" || charttype == "BasicChart" || charttype == "PieChart" || charttype == "GaugeChart" || charttype == "FloatChart") {
					if (!categorycolumn) {
						invaliddata = true;
					}
					else {
			...
			if (dataMap) {
				if (valuecolumn && !value2column && !value3column && charttype != "FloatChart") {
							...
							if (obj["key"]) {
								...
								if (rotateaxis && !chart._isCompositeSeries) {
									...
									if (charttype == "PyramidChart") {
										data.push([categorydata, obj[valuecolumn], pyramidindex]);
										pyramidindex++;
									}
									else {
				...
				if ((valuecolumn && value2column) || (charttype == "FloatChart" && valuecolumn && chart.waterfall)) {
					var waterfallSum = 0;
					var waterfallIndex;
									...
									if (charttype == "FloatChart") {
										...
										if (rotateaxis) {
											if (chart.waterfall) {
												waterfallSum += xdata;
												if (waterfallPreData) {
													data.push([xdata + waterfallPreData[0], categorydata, waterfallPreData[0]]);
												}
												else {
													data.push([xdata, categorydata, 0]);
												}
											}
											else {
												data.push([xdata, categorydata, ydata]);
											}
										}
										else {
											if (chart.waterfall) {
												waterfallSum += xdata;
												if (waterfallPreData) {
													data.push([categorydata, xdata + waterfallPreData[1], waterfallPreData[1]]);
												}
												else {
													data.push([categorydata, xdata, 0]);
												}
											}
											else {
												data.push([categorydata, xdata, ydata]);
											}
										}
			...
			if (charttype == "FloatChart" && chart.waterfall && data.length > 0) {
				if (rotateaxis && data.length) {
					data.push([waterfallSum, chart.waterfallsumtext, 0]);
				}
				else {
					data.push([chart.waterfallsumtext, waterfallSum, 0]);
				}
			}
			if (charttype == "PyramidChart") {
				function sortAFunction (a, b) {
					return a[1].valueOf() - b[1].valueOf();
				}
				function sortDFunction (a, b) {
					return b[1].valueOf() - a[1].valueOf();
				}
				if (this._chart.invertchart === true) {
					if (this._chart.graphtype == "pyramid") {
						data.sort(sortDFunction);
					}
					else {
						data.sort(sortAFunction);
					}
				}
				else {
					if (this._chart.graphtype == "pyramid") {
	...
	_pSeriesBase._selectItem = function (index, selectType, selectColumn, selectedItem, obj) {
		var re = true;
		if (nexacro._ChartGraphicsLib.isEmpty(index)) {
			return false;
		}
		...
		if (selectType == "unselect") {
			...
			if (nexacro._ChartGraphicsLib.isEmpty(selectColumn) || nexacro._ChartGraphicsLib.isEmpty(obj)) {
				re = false;
			}

			if (!nexacro._ChartGraphicsLib.isBoolean(selectColumn)) {
	...
	_pSeriesBase._end_animation_series_callback = function () {
		...
		if (endanimation == serieslength) {
			...
			if (befo != chart._isanimationloading) {
				chart._graphicsControl.draw();
				chart._drawStackTotal();
				this._chart._drawEndEvent();
			}
		}
	};
	...
	_pSeriesBase._getanimationdrawvalue = function (value) {
		...
		var gap = 0, min = 0, calcval = 0, ps = 0, i;
		if (!nexacro._ChartGraphicsLib.isArray(value)) {
			switch (type_name) {
				case "FloatChart":
				case "BasicChart":
					{
...
if (!nexacro.ChartBoardControl) {
	...
	_pChartBoardControl._createGridRect = function () {
		this._gridRect = new nexacro.ChartGraphicsRect(0, 0, 0, 0);
	...
	_pChartBoardControl._createCenterText = function () {
		if (!this._gridCenterText) {
			if (this._group) {
				this._gridCenterText = new nexacro.ChartGraphicsText(0, 0);
...
if (!nexacro.ChartTitleControl) {
	...
	_pChartTitleControl._createGroup = function () {
		this._titleGroup = new nexacro.ChartGraphicsGroup();
		this._titleGroup.set_id("ChartTitleGroup");
		this._graphicsControl.addChild(this._titleGroup);

		this._titleRect = new nexacro.ChartGraphicsRect(0, 0, 0, 0);
	...
	_pChartTitleControl._createTitleText = function () {
		this._titleText = new nexacro.ChartGraphicsText(0, 0);
	...
	_pChartTitleControl._createSubTitleText = function () {
		this._subTitleText = new nexacro.ChartGraphicsText(0, 0);
	...
	_pChartTitleControl._measure = function (width, height) {
		var txtRect, subtxtRect, titleRect = this._titleRect, paddingLeft = 0, paddingRight = 0, paddingTop = 0, paddingBottom = 0, padding = this._padding, linestyle = this._linestyle, position = this._position || "top";
...
if (!nexacro.ChartLegendControl) {
	...
	_pChartLegendControl.on_apply_itemtextcolor = function (itemtextcolor) {
		...
		var items = this._items, itemsLen = items.length, label, data, seriesVisible = true, iteminvisiblecolor = this._iteminvisiblecolor, itemGroup;
	...
	_pChartLegendControl._createGroup = function () {
		...
		this._backgroundRect = new nexacro.ChartGraphicsRect(0, 0, 0, 0);
		...
		this._itemsGroup = new nexacro.ChartGraphicsGroup();
	...
	_pChartLegendControl._setData = function (seriesset) {
		this._clearItems();
		if (nexacro._ChartGraphicsLib.isEmpty(seriesset)) {
			this._data = [];
			return;
		}

		var charttype = this.parent._type_name, i, length = 0, seriesitem, ldata, data = [];

		if (charttype == "BasicChart" || charttype == "BubbleChart" || charttype == "RadarChart" || charttype == "GaugeChart" || charttype == "RoseChart") {
			length = seriesset.length;
			for (i = 0; i < length; i++) {
				var series = seriesset[i], visible = false, fillstyle;
				ldata = {
				};
	...
	_pChartLegendControl._clearItems = function () {
		...
		for (var i = 0; i < this._items.length; i++) {
			itemsGroup = this._items[i];
			if (itemsGroup) {
				itemsGroup.clear();
			}
		}

		itemsGroup = this._itemsGroup;
		if (itemsGroup) {
			itemsGroup.clear();
		}
	...
	_pChartLegendControl._createItems = function () {
		...
		for (var i = 0; i < dataLen; i++) {
			...
			itemGroup = new nexacro.ChartGraphicsGroup();
			...
			marker._legend = this;
			label = new nexacro.ChartGraphicsText(0, 0);
	...
	_pChartLegendControl._createMarker = function (markertype, markersize) {
		var marker;
		if (markertype == "circle") {
			marker = new nexacro.ChartGraphicsEllipse(markersize * 0.5, markersize * 0.5, markersize, markersize);
		}
		else {
			marker = new nexacro.ChartGraphicsRect(0, 0, markersize, markersize);
		}
...
if (!nexacro.ChartTooltipControl) {
	...
	_pChartTooltipControl.edgeradius = 0;
	...
	_pChartTooltipControl.set_edgeradius = function (val) {
		if (val !== undefined) {
			if (isNaN(val) || val < 0) {
				return;
			}
		...
		if (this.edgeradius !== val) {
			this._changeContentsProperty("edgeradius", val, this.edgeradius);
			this.edgeradius = val;
			this.on_apply_edgeradius(val);
		}
	};

	_pChartTooltipControl.on_apply_edgeradius = function (edgeradius) {
	};
	...
	_pChartTooltipControl._createTooltip = function () {
		if (!this._tooltipPath) {
			this._tooltipPath = new nexacro.ChartGraphicsPath();
			this._tooltipPath.set_id("ChartTooltipPath");
			this._group.addChild(this._tooltipPath);
		}

		if (!this._tooltipText) {
			this._tooltipText = new nexacro.ChartGraphicsText();
	...
	_pChartTooltipControl._showTooltip = function (obj, canvasX, canvasY) {
		...
		if (!this.background) {
			if (obj.fillstyle) {
				this.on_apply_background(obj.fillstyle);
			}
			else {
				this.on_apply_background("#4b4b4b");
			}
		}
		else {
			this.on_apply_background(this._background);
		...
		var nRoundPix = this.edgeradius;
		if (((tooltipWidth / 2) < nRoundPix) || ((tooltipHeight / 2) < nRoundPix)) {
			nRoundPix = 0;
		}
		...
		var fst_top = top;
		if (nRoundPix > 0) {
			tooltipPath.moveTo(left + nRoundPix, top);
		}
		else {
		...
		left += tooltipWidth;
		if (nRoundPix > 0) {
			tooltipPath.lineTo(left - nRoundPix, top);
		}
		else {
		...
		top += tooltipHeight;
		if (nRoundPix > 0) {
			tooltipPath.quadraticCurveTo(new nexacro.Point(sv_left - nRoundPix, sv_top), new nexacro.Point(sv_left, sv_top + nRoundPix));
			tooltipPath.lineTo(left, top - nRoundPix);
		}
		else {
			tooltipPath.lineTo(left, top);
		}
		...
		if (nRoundPix > 0) {
			tooltipPath.quadraticCurveTo(new nexacro.Point(sv_left, sv_top - nRoundPix), new nexacro.Point(sv_left - nRoundPix, sv_top));
			tooltipPath.lineTo(left, top);
		}
		else {
			tooltipPath.lineTo(left, top);
		...
		if (nRoundPix > 0) {
			tooltipPath.lineTo(sX + nRoundPix, top);
			tooltipPath.quadraticCurveTo(new nexacro.Point(sX + nRoundPix, top), new nexacro.Point(fst_left, fst_top + tooltipHeight - nRoundPix));
			tooltipPath.lineTo(fst_left, fst_top + nRoundPix);
			tooltipPath.quadraticCurveTo(new nexacro.Point(fst_left, fst_top + nRoundPix), new nexacro.Point(fst_left + nRoundPix, fst_top));
		}
		else {
	...
	_pChartTooltipControl._moveTooltip = function (obj, canvasX, canvasY, boardRect) {
		...
		if (overitemid.search("SeriesItemText_") >= 0) {
			if (obj._series) {
				...
				if (this.parent._type_name == "RadarChart" || this.parent._type_name == "RoseChart") {
					s = obj._series;
					if ((!s.linevisible && !s.pointvisible)) {
						return;
					}
		...
		if (obj._tooltipShow) {
			...
			}, txtWidth = 0, txtHeight = 0, paddingLeft = 0, paddingRight = 0, paddingTop = 0, paddingBottom = 0, padding = this._padding, linestyle = this._linestyle, borderwidth = 0, tooltipWidth = 0, posRight = 0, posExtLeft = 0, posExtRight = 0, bound, hookheight, tooltipOffsetY = this._offsety, tooltipOffsetX = this._offsetx;
	...
	_pChartTooltipControl._getTooltipText = function (item) {
		var series, index, tooltiptext, tooltiptexttype, tooltiptextmask, chart = this.parent, locale, textvalue;
		...
		tooltiptext = nexacro._getChartDisplaytText(index, tooltiptext, locale, tooltiptexttype, tooltiptextmask, chart, this, series);
...
if (!nexacro._getChartDisplaytText) {
	nexacro._getChartDisplaytText = function (idx, text, locale, type, mask, chart, targetcontrol, series) {
		var displaytext;
	...
	nexacro._getChartAttrValue = function (attr, rowidx, mask, chart, series) {
		if (attr == undefined) {
			return undefined;
		}
		...
		if (attr._bindtype == undefined) {
			...
			if (attr._bindtype == 1) {
				if (chart._type_name == "FloatChart") {
					if (((dataset.rowcount - 1) < rowidx) && series && series._data && series._data[rowidx]) {
						if (chart.rotateaxis) {
							return series._data[rowidx][0];
						}
						else {
							return series._data[rowidx][1];
						}
					}
				}
	...
	nexacro._getChartDisplayFormatText_date = function (date, mask) {
		if (date instanceof nexacro.Date) {
		}
		else {
			return date;
		}
		if (!date || date == "" || !mask || mask == "") {
			return v;
		}
		var dateStr;
		var format;
	...
	nexacro._getChartDisplayText_datetime2 = function (value, rowidx, locale, mask, chart) {
		if (mask && mask.length) {
			...
			if (value != null) {
				formatvalue = nexacro._getChartDisplayFormatText_date(value, mask);
			}
...
if (!nexacro._getBeforeKey) {
	...
	nexacro._getBeforeKey = function (curKey, isRetChrArr) {
		var charCodeArr;
		if (!nexacro._ChartGraphicsLib.isArray(curKey)) {
...
if (!nexacro._DatasetMap) {
	...
	_pDatasetMap.findRowByKey = function (key) {
		...
		if (data) {
			if (this._allowDuplicates) {
				if (nexacro._ChartGraphicsLib.isArray(data) && data._dataArray === true) {
					data = data[0];
				}
			}
	...
	_pDatasetMap.insert = function (row, key, data, cnt) {
		...
		if (this._allowDuplicates && this.containsKey(key)) {
			var dataArr = map[key];
			if (nexacro._ChartGraphicsLib.isArray(dataArr) && dataArr._dataArray === true) {
				dataArr.push(data);
			}
			else {
	...
	_pDatasetMap.removeAt = function (row) {
		...
		if (data) {
			...
			if (this._allowDuplicates && this.containsKey(key)) {
				var dataArr = map[key];
				if (nexacro._ChartGraphicsLib.isArray(dataArr) && dataArr._dataArray === true) {
					var idx = nexacro._ChartGraphicsLibArray.indexOf(dataArr, data, 0, true);
					if (idx > -1) {
	...
	_pDatasetMap.buildDataWithDataset = function (ds, keyColumnId, createDataFunc, scope) {
		...
		for (var i = 0, len = ds.rowcount; i < len; i++) {
			if (!nexacro._ChartGraphicsLib.isEmpty(keyColumnId)) {
	...
	_pDatasetMap.changeKey = function (data, key, row, createDataFunc, scope) {
		var oldKey = data.key, dataArr, map = this.keyMap;
	...
	_pDatasetMap.moveRow = function (oldRow, newRow) {
		...
		if (oldDataAtRow) {
			...
			if (this._allowDuplicates && this.containsKey(key)) {
				var dataArr = map[key];
				if (nexacro._ChartGraphicsLib.isArray(dataArr) && dataArr._dataArray === true) {
					var idx = nexacro._ChartGraphicsLibArray.indexOf(dataArr, oldDataAtRow, 0, true);
					if (idx > -1) {
						dataArr.splice(idx, 1);
						if (!dataArr.length) {
							delete map[key];
...
if (!nexacro._BTree) {
	nexacro._BTree = function (compare, capacity) {
		if (nexacro._ChartGraphicsLib.isFunction(compare)) {
	...
	_pBTree._nodeFind = function (r, key, keycompare, duplicatesBias, refVal) {
		var arrayUtil = nexacro._ChartGraphicsLibArray, tmpRefval = {
		};
	...
	_pBTree._movecopy = function (source, sourceIndex, sourceTotal, target, targetIndex, targetTotal, count) {
		var arrayUtil = nexacro._ChartGraphicsLibArray;
